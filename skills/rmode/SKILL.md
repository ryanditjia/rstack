...
Let the current thread choose direct work, subagents, or agent threads from the workload. Treat fixed agent counts, model roles, and delegation layouts in this skill as coverage or independence goals, not commands. Keep synthesis, judgment, and the final answer in the current thread.
...
Before delegating, read [references/host-capabilities.md](references/host-capabilities.md). Use the host's agent mechanism and run independent work in parallel when supported. Use configured model roles only when the host supports them; otherwise inherit the current model. Pass file pointers instead of large copied payloads.
