...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Implement directly or delegate a bounded fix. Have a reviewer who did not write the fix inspect the diff; if you delegate implementation, you can provide that review. Capture a post-fix trace.
