...
- **Worker / verifier.** Workers run in child sessions. Give a worker this machine when the task needs it: host UI or CLI control capability verification, local transcripts, simulators and local IDE state, or auth that exists only here. Otherwise point at repo paths and inline what it needs, because a child session cannot read the parent's context. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different agent from its worker.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape. A 4KB scaffold around a two-line edit costs more to write and obey than the edit. Spawns may reference the standing-orders file by store path. Paste verbatim on every resume.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the default agent and the exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- Exactly one stacker per stack may run the stack tool, serialized within its stack. Record the holder in the standing orders. Restacks run serialized in a child session.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the session's status. Transcript mtime is not liveness.
