...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, and makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are worker tasks. It may mechanically land a verified unit only when pushes are authorized. Agents are spawned, resumed, and drained through the host's delegation mechanism. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
...
- **Worker / verifier.** Use a separate environment when available unless the task needs this machine for UI or CLI runtime verification, workspace-scoped task history, simulators, local IDE state, or local authentication. Remote workers cannot assume access to local state, so their briefs inline what they need or point at repository paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Prefer a verifier from a different model family when the host supports model selection.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so completion traffic cannot obscure a pending user decision.
...
- Never resume an agent merely to check on it when resume restarts idle work. Probe read-only through the ledger, `units.tsv`, `gh`, pushed branches, and the host's agent-status interface. Transcript mtime is not liveness.
