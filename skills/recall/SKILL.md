...
Use the host's task-history or transcript interface when it exposes one. Do not assume a fixed filesystem layout. If history is unavailable, work from the current conversation and any capsule or transcript the user provides.
...
3. Search the available chat history. If the host supports parallel delegation, assign bounded date or topic slices; otherwise search sequentially. Order candidates by real modification time when files are available, search for the topic first, and read only relevant regions. Skip the current conversation plus obvious evaluation and test noise. Return one block per chat: topic, user goal, decisions, open threads, corrections, and artifacts such as PRs, tickets, or branches. For one or two chats, search directly.
...
An adjacent feature or ticket stays out unless it blocks this one. When the capsule and thread lines outgrow a screen, cut detail before you cut threads. Write plainly, cite chat findings by UUID and shared-record findings by their source (PR #, ticket ID, chat permalink, error-tracker issue), and sanitize private context before any public output.
