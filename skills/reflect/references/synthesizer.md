...
- Existing-skill-first: propose `new skill via authoring:` only when no existing skill is a real home, the pattern recurs, and the topic deserves its own skill.
...
| <new pattern, no existing skill is a real home> | <draft a new skill via authoring> | <new skill via authoring: <kebab-name>> |
