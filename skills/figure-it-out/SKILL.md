...
Open a todolist whose first item is to read the Principles section of the **rstack-mode** skill. Then add the phases below as todos.
...
- For one-way-door design decisions, run the **architect** skill. It chooses direct design work, bounded delegation, or an arena from the uncertainty. Skip it for mechanical work whose shape is already concrete. Re-running exploration over a settled design is over-engineering (the **laziness-protocol** principle skill).
