...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Use `why` to investigate the relevant PR and commits.
...
6. For a big or wide change, get an independent review of the affected boundaries and safety claims. Start with one read-only reviewer given the change, relevant sources, and proof artifacts. Add reviewers for a named coverage gap or unresolved disagreement. The current thread reconciles findings and verifies material claims against the real code. Use `arena` when competing complete investigations would resolve a material uncertainty, or when the user requests it.
...
Write plainly, cite real code, and strip anything private before it goes anywhere public.
