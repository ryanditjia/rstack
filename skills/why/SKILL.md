...
Before starting investigators, list the integrations and tools exposed by the current host. Use its available-tools map, resource listing, connector discovery, or equivalent capability. Do not assume a specific directory layout.
...
Run each investigator independently. Use the optional `why-investigators` model preference when supported; otherwise inherit the current model. Grant only the read access needed for the relevant source. If the host ties integrations to an interactive mode, use that mode while explicitly forbidding writes.
...
Synthesize through one independent pass, delegated when supported. Use the optional `why-synthesizer` model preference or inherit the current model. Give it read access to cited sources so it can spot-check citations, but do not permit writes.
