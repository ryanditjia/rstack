---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill via authoring + unslop, optionally pulling fresh evidence from recent transcripts."
...
This skill orchestrates three others: an inline mining pass (see step 1), the host's skill-authoring guidance (authoring), and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.
...
Locate the active workspace's transcripts before fanning out. The system prompt names the workspace's session store. Use only that source. Don't glob across other workspaces' transcripts. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Use the host's skill-authoring flow to author the skill. Placement:
...
Apply the **unslop** skill and the authoring guidance's writing rules to every line.
