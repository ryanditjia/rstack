...
This skill orchestrates three activities: an inline mining pass, the host's available skill-authoring workflow, and a prose-discipline pass. It sequences them; it does not replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.agents/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.agents/skills/<handle>/`), not only at the top level. If one exists, ask whether to update it unless the user already made that intent clear:
...
Use the host's task-history interface if one is available and scoped to the active workspace. Do not search unrelated workspaces. If history is unavailable, use the current conversation and evidence the user provides.

Survey recent agent conversations within that scope for recurring patterns. If delegation is supported, review bounded slices in parallel; otherwise review them sequentially. Each pass returns a short structured list with evidence pointers. Default signals worth hunting:
...
Mining misses intent that has not come up yet. Use structured choices when the host supports them; otherwise ask concise direct questions.
...
Use the host's available skill-authoring guidance to author the skill. Placement:
...
- Frontmatter formatting: follow the Agent Skills specification and the host's skill-authoring guidance. Keep `description` as one YAML scalar; quote it or use `description: >-` with indented continuation lines when punctuation or wrapping requires it.
...
Save the skill in the requested scope. Commit or open a PR only when the user asked for repository changes or the surrounding workflow already authorizes them.
...
A `-mode` skill is subjective output. A benchmark loop is not useful here. Check the draft with the user: does it read like them, and did it miss anything? Then ship.

Run a description-optimization loop only if the skill's trigger accuracy turns out to be a problem in practice.

## When not to use

- User wants a task-specific skill rather than working conventions: use the host's skill-authoring workflow without transcript mining.
