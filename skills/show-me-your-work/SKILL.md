...
Write each entry the way you'd tell a teammate what you did. Use plain words, concrete actions, and no abstract jargon. A reviewer should understand each row without decoding it.
...
At the end of the run, before handing back, check that the log tells the truth. Use the host's active-task transcript interface when available; otherwise audit against the current conversation and command evidence. Never search unrelated workspaces. Walk the log against what actually happened:
...
Before handing back, seek an independent review when the host supports delegation. Prefer a different model family when one is configured, but do not fail the workflow when model selection is unavailable. If delegation is unavailable, do a separate second-pass review and say so. The reviewer reads the audit trail and the run's transcript or digest, then flags what the user should pay attention to.
...
Every reply for a run that produced a trail ends with an "Attention" section. Identify the reviewer or review method, then list each flag pointing to specific rows or moments. "No flags" is valid. The self-audit asks if the log told the truth; this asks what the user should still scrutinize even when it did.
