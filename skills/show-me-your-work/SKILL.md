...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript from the host's session store (the system prompt names the location). Don't glob across other workspaces' transcripts. That reads unrelated private chats. Walk the log against what actually happened:
